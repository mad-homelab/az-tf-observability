# Module: State Storage (Backend Bootstrap)
...
| :--- | :--- | :--- | :--- |
| `service_name` | `string` | **Required** | Name of service provided by resources. |
| `resource_group_name` | `string` | **Required** | Resource Group name. |
| `storage_account_name` | `string` | **Required** | Custom identifiers for the storage account name (3-24 chars). |
| `container_name` | `string` | **Required** | Blob container name for Terraform state (3-24 chars). |
| `rg_location` | `string` | `australiasoutheast` | The Azure Region in which the RG exists. |
| `resource_location` | `string` | `australiasoutheast` | The Azure Region in which the Resource exists. |
| `environment` | `string` | `dev` | Target environment (Validation: `dev` or `test`). |
| `account_tier` | `string` | `Standard` | Performance tier for the storage account. |
| `account_replication_type` | `string` | `LRS` | Data redundancy strategy (LRS, GRS, etc.). |
| `allow_public_access` | `bool` | `true` | Whether the storage is accessible from public networks. |
| `shared_access_key_enabled`| `bool` | `false` | **Security Toggle**: Disables shared access keys. |
